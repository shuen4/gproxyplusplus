	...
	while( !m_Packets.empty( ) )
	{
		...
		if( Packet->GetPacketType( ) == BNET_HEADER_CONSTANT )
		{
			...
			case CBNETProtocol :: SID_AUTH_INFO:
				...
				{
					if( m_BNCSUtil->HELP_SID_AUTH_CHECK( m_GProxy->m_TFT, m_War3Version, m_GProxy->m_War3Path, m_CDKeyROC, m_CDKeyTFT, m_Protocol->GetValueStringFormulaString( ), m_Protocol->GetIX86VerFileNameString( ), m_Protocol->GetClientToken( ), m_Protocol->GetServerToken( ) ) )
					{
